function [E,F,G] = obstaclesComputeCustomConstraint(x,detections,obstacles,laneWidth,lanes)
%% Compute custom constraints for each obstacle.
...
for i=1:length(detections)
    ...
    constraintSlopes = [constraintSlopes; constraintSlope];
    constraintIntercepts = [constraintIntercepts; constraintIntercept];
end
%% Define constraint matrices.
E = [0 0;0 0];
F = [0 1 0 0;0 -1 0 0];
G = [laneWidth*lanes/2;laneWidth*lanes/2;];
for i=1:length(constraintSlopes)
    E = [E;0 0];
    F = [F;constraintSlopes(i) -1 0 0];
    G = [G;-1*constraintIntercepts(i)];
        %E = [0 0;0 0;0 0];
